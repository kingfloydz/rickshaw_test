...
路径命令固定为：`v_ref`、机器人与车辆中点的横向路径误差 `e_y`、机身航向相对坡面切向的误差 `e_psi`。零 yaw-rate 不再作为任务目标，因为它不能约束累计航向偏差。训练环境在 startup 为 8192 个环境分别采样躯干质量、车辆载荷质量/CoM、滚阻、摩擦和轮阻尼，并写入真实物理；坡度同样在 startup 均衡随机分配。所有这些参数在训练期间保持不变，普通 episode reset 不重新采样。电机强度、逐关节模型误差、控制延迟和观测延迟均不随机。8 个 D6 参数固定为 nominal，以保留 `replicate_physics=True` 的并行吞吐。评估和确定性诊断使用同一配置的 nominal 模式。
...
冻结 teacher。由 teacher 在同一个 `TRAINING` 分布中固定采集 `8192*64=524,288` 条 on-policy transition；该离线预算不随 PPO 的 R 改变。Student 使用同一 `current` 和过去 61 帧 history，latent 维度 D 从 S0 checkpoint 继承。Student actor 先严格复制 teacher actor并冻结，S1 只优化 student context encoder：
