...
```text
student: history [61,96] -> 1x1 projection 64 -> causal TCN 64 -> z_hat 16

teacher: history [61,96] ---------> 1x1 projection 64 --+
         dynamic [61,21] ---------> 1x1 projection 64 --+-> sum -> causal TCN 64 --+
         static [40] -> Linear 32 + ELU ----------------------------------------+-> Linear 16 -> z_star

actor:  current 96 + z 16 -> [512,256,128] -> Gaussian 29
critic: current 96 + raw privilege 64 -> [256,128] -> value
```

`current` 提供即时反馈；`z_hat` 从过去响应提取步态和车辆相位等时序状态。Actor 输入只能是 `concat(current,z)`，维度 `112`。
...
Teacher 的 observation 与 dynamic 分支先分别做 `1x1` 投影，再逐元素相加进入同一组 4 层 causal blocks；static 经过 `40 -> 32 + ELU`，与最后时刻的 64 维 temporal feature 拼接成 96 维并线性输出 `z_star=16`。Teacher 与 student 不包含 auxiliary head、projection adapter 或可调 latent 维度。

Actor MLP 固定为 `112 -> 512 -> 256 -> 128 -> 29`，ELU；Gaussian log-std 为独立参数，lower-body 初始 std `0.4`、waist/arm 初始 std `0.25`。Critic 固定为 `160 -> 256 -> 128 -> 1`，直接读取原始 64 维 privilege，不接收 latent，也不与 actor 共享 trunk。
...
默认前 `1000` iterations 为 `STATIC_HAND_LOAD`，之后为 `TRAINING`，总计最多 `6000` iterations。阶段切换只在单个环境 reset 时生效，避免 episode 中途改变动力学。这里没有 off-policy replay buffer。4096 个环境在 iteration 0 完成一次 domain 采样并固定；每 200 iterations 只在 PPO update 边界重新采样并同步 reset 全部环境，普通 episode reset 不采样。Checkpoint 按固定周期保存；评估作为独立诊断运行，不控制训练启动、停止或恢复。

### 10.2 S1 On-policy Student Distillation

冻结 teacher。由 teacher 在同一个 `TRAINING` 分布中生成 on-policy rollout；student 用同一 `current` 和过去 61 帧 history。Student actor 先严格复制 teacher actor 并冻结，S1 只优化 student context encoder：
...
用 `z_hat` 替换 `z_star`，critic 继续读取独立 raw privilege group。Context encoder 与 actor 在 S2 一并解冻：context learning rate `1e-4`，actor/critic learning rate `3e-4`，最多继续 PPO `2000` iterations。S2 不再保留 distillation loss。
...
主线固定每次 rollout 为 `48` steps/env，即 `0.96 s`、`196,608` transitions；8 个 mini-batch 时每个为 `24,576`。
...
不为 wheel contact、wheel height、D6 residual、D6 asymmetry、arm saturation、overspeed 或 cart pitch-rate设置额外 dense reward；这些量进入 barrier、termination 和日志。机器人 pelvis 高度是独立的姿态可行域约束：以坡面法向距离计算，区间内不计分，只惩罚越界距离，避免与目标高度跟踪重复。hip yaw/roll 只以小权重跟随各坡度静力学 `q_ref`，不约束承担牵引姿态的 hip pitch。FAT2 权重固定为 `+0.1`，且不得高于 ZMP barrier 的绝对权重。
...
| 训练分布 | 默认迭代 | 配置 |
| --- | ---: | --- |
| `STATIC_HAND_LOAD` | `0..999` | 不使用真实 rickshaw；关闭双 D6，将车辆停放并禁用重力；在双手施加 reset pose 静力解，均衡覆盖 `-0.08` 到 `+0.10` 的 19 个坡度。 |
| `TRAINING` | `>=1000` | 恢复真实 rickshaw、双 D6 和滚阻；使用当前 200-iteration domain epoch 的物理参数，D6 保持 nominal；阶段只在 episode reset 时切换。 |
