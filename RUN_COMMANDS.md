...
以下命令给出从资产准备到最终播放/导出的完整顺序。训练产生的 checkpoint
文件名由 RSL-RL 日志目录决定，需要将占位符替换成实际路径。

资产检查报告、reset alignment 报告和 feasibility/dynamics 扫描均为可选诊断，
不参与训练启动、续训、消融汇总或 checkpoint lineage。训练入口直接加载实际配置
和资产；加载失败会直接报错。
...
# 2. 可选：检查源资产与组合后的 USD physics ABI
...
- `outputs/validation/asset_inspection.json`（可选诊断）
...
选择。S1 上限为 4000 iterations；S0、S1、S2 均每 200 iterations 检查，连续
5 次无提升提前停止。S0/S2 使用固定种子 task return，S1 使用固定验证集 action KL：
...
latent dim 消融允许值为 `8`、`16`、`24`、`32`。已有 rollout 时可显式指定：
...
消融矩阵必须包含恰好 10 个矩阵项：FAT2 weight `0/0.1/0.2`、rollout steps
`24/48/64`、latent dim `8/16/24/32`。物理滚阻在全部训练、评估和播放中始终开启，
保留真实轮心切向阻力实现，不再作为 `off/on` 消融因素。每个矩阵项需绑定对应的
S2 checkpoint、S0 teacher 和 S1 baseline report。

三个默认矩阵项共用一套 baseline，因此实际训练以下 8 个唯一配置：
...
| `fat2_weight_0.2` | 0.2 | 48 | 16 | `fat2_weight_0.2` |
...
| `latent_dim_32` | 0.1 | 48 | 32 | `latent_dim_32` |
...
然后在 8 个计算节点分别设置对应的 `RUN_ID`，每个节点只执行一次相同命令：
...
该命令不重新训练；它要求 8 个唯一配置均通过验证，然后生成 10 项正式矩阵、运行
...
省略 `--runs` 表示运行全部 8 个唯一配置。也可以显式限制配置；非
`--worker-only` 模式只有在全部 8 个配置完成后才会生成正式矩阵：

```bash
"$PYTHON" scripts/run_ablation_pipeline.py \
  --final-thresholds config/final_thresholds.yaml \
  --output-dir outputs/ablation_pipeline \
  --runs latent_dim_8 latent_dim_24 latent_dim_32 \
  --gpus 0 1 2 \
  --resume \
  --skip-postprocess
```
...
- `outputs/ablation_pipeline/results/evaluation/*.json`：10 个矩阵项的正式评估报告；
...
`--video-length`、`--video-num-envs` 调整录像，使用 `--skip-video` 只生成数据结果。

也可以直接调用 `evaluate_policy.py` 运行单个消融，例如 latent dim 32：

```bash
"$PYTHON" scripts/evaluate_policy.py \
  --checkpoint <LATENT_32_S2_CHECKPOINT> \
  --teacher-checkpoint <LATENT_32_S0_CHECKPOINT> \
  --s1-baseline-report <LATENT_32_S1_REPORT.json> \
  --output outputs/policy_ablations/latent_dim_32.json \
  --thresholds config/final_thresholds.yaml \
  --ablation-id latent_dim_32 \
...
- `inspect_assets.py` 只生成离线诊断结果；其报告缺失或过期不会阻止训练、续训、
  播放、导出或消融汇总。
